    ...
    <section className="grid gap-6 lg:grid-cols-[1.4fr_0.8fr]">
      <div className="rounded-lg border border-slate-200 bg-white p-6 shadow-soft">
        ...
        <div className="mt-8 space-y-3">
          ...
          >
            {enterMutation.isPending ? '대기열 입장 중' : isTicketOpen ? '예매하기' : '예매 오픈 전'}
          </button>
        </div>
      </div>
      <aside className="rounded-lg border border-slate-200 bg-white p-6">
        ...
        <div className={`mt-5 rounded-lg p-5 text-center ${isTicketOpen ? 'bg-green-50' : 'bg-amber-50'}`}>
          <p className={`text-sm font-bold ${isTicketOpen ? 'text-green-700' : 'text-amber-700'}`}>
            {isTicketOpen ? '예매 가능' : '예매 오픈까지'}
          </p>
          <p className={`mt-2 text-3xl font-black ${isTicketOpen ? 'text-green-800' : 'text-amber-800'}`}>
            {isTicketOpen ? 'OPEN' : countdown}
          </p>
        </div>
